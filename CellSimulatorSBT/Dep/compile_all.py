...
# enable_packages = ['boost', 'eigen', 'pvfmm', 'trilinos', 'vtk']
enable_packages = ['pvfmm']
...
# PVFMM
if 'pvfmm' in enable_packages:
    pvfmm = 'af175f382320e6de8c73afa8133b9ec32a433145'
    os.chdir(depwd)
    run('rm -rf ./build_pvfmm && mkdir ./build_pvfmm')
    # run('tar xf {}.tar.gz'.format(pvfmm))
    os.chdir('build_pvfmm')
    run('bash ../cmake-pvfmm.sh && make -j' +
        str(make_jobs)+'  >> '+log+'  2>>'+err)
    if install:
        run('make install'+' >> '+log+'  2>>'+err)
