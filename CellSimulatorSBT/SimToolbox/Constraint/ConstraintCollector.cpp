...
#pragma omp parallel for num_threads(nThreads)
    ...
    const GO mobMin = mobMapRcp->getMinGlobalIndex();
    const GO mobMax = mobMapRcp->getMaxGlobalIndex();
    std::vector<GO> colMapIndex(mobMax - mobMin + 1);
#pragma omp parallel for
    for (GO i = mobMin; i <= mobMax; i++) {
    ...
    const GO colIndexNum = columnIndices.extent(0);
    if (colIndexNum != colIndexCount) {
        printf("colIndexNum error");
        std::exit(1);
    }
    for (GO i = 0; i < colIndexNum; i++) {
...
#pragma omp parallel for
    ...
    // printf("local number of cols: %d on rank %d\n", colMapRcp->getLocalNumElements(), commRcp->getRank());
    // printf("local number of rows: %d on rank %d\n", mobMapRcp->getLocalNumElements(), commRcp->getRank());
    ...
    auto delta0 = delta0Rcp->getLocalView<Kokkos::HostSpace>(Tpetra::Access::ReadWrite);
    auto gammaGuess = gammaGuessRcp->getLocalView<Kokkos::HostSpace>(Tpetra::Access::ReadWrite);
    auto invKappa = invKappaRcp->getLocalView<Kokkos::HostSpace>(Tpetra::Access::ReadWrite);
    auto biFlag = biFlagRcp->getLocalView<Kokkos::HostSpace>(Tpetra::Access::ReadWrite);

#pragma omp parallel for num_threads(cQueNum)
    for (size_t que = 0; que < cQueNum; que++) {
...
int ConstraintCollector::writeBackGamma(const Teuchos::RCP<const TV> &gammaRcp) {
    ...
    auto gammaPtr = gammaRcp->getLocalView<Kokkos::HostSpace>(Tpetra::Access::ReadOnly);
#pragma omp parallel for num_threads(cQueNum)
    for (size_t i = 0; i < cQueNum; i++) {
        const int cQueSize = cPool[i].size();
        for (size_t j = 0; j < cQueSize; j++) {
