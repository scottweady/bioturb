...
BCQPSolver::BCQPSolver(int localSize, double diagonal) {
    ...
    const GO colIndexCount = rowPointers[localSize];
    std::vector<GO> colMapIndex(colIndexCount);
#pragma omp parallel for
    for (GO i = 0; i < colIndexCount; i++) {
    ...
    Teuchos::RCP<TMAP> colMapRcp = Teuchos::rcp(
        new TMAP(Teuchos::OrdinalTraits<GO>::invalid(), colMapIndex.data(), colMapIndex.size(), 0, commRcp));
...
int BCQPSolver::solveBBPGD(const Teuchos::RCP<TV> &xsolRcp, const double tol,
...
int BCQPSolver::solveAPGD(const Teuchos::RCP<TV> &xsolRcp, const double tol,
                          const int iteMax, IteHistory &history) const {
    // map must match
    TEUCHOS_TEST_FOR_EXCEPTION(!this->mapRcp->isSameAs(*(xsolRcp->getMap())), std::invalid_argument,
                               "xsolrcp and A operator do not have the same Map.")
...
void BCQPSolver::boundProjection(Teuchos::RCP<TV> &vecRcp) const {
    ...
    auto vecPtr = vecRcp->getLocalView<Kokkos::HostSpace>(Tpetra::Access::ReadWrite); // LeftLayout
    ...
    auto lbPtr = lbRcp->getLocalView<Kokkos::HostSpace>(Tpetra::Access::ReadOnly);
#pragma omp parallel for
    ...
    auto ubPtr = ubRcp->getLocalView<Kokkos::HostSpace>(Tpetra::Access::ReadOnly);
#pragma omp parallel for
    for (size_t i = 0; i < ibound; i++) {
        const double temp = vecPtr(i, c);
...
double BCQPSolver::checkProjectionResidual(const Teuchos::RCP<const TV> &XRcp, const Teuchos::RCP<const TV> &YRcp,
    ...
    auto xPtr = XRcp->getLocalView<Kokkos::HostSpace>(Tpetra::Access::ReadOnly);   // LeftLayout
    auto yPtr = YRcp->getLocalView<Kokkos::HostSpace>(Tpetra::Access::ReadOnly);   // LeftLayout
    auto lbPtr = lbRcp->getLocalView<Kokkos::HostSpace>(Tpetra::Access::ReadOnly); // LeftLayout
    auto ubPtr = ubRcp->getLocalView<Kokkos::HostSpace>(Tpetra::Access::ReadOnly); // LeftLayout
    auto qPtr = QRcp->getLocalView<Kokkos::HostSpace>(Tpetra::Access::ReadWrite);   // LeftLayout
...
void BCQPSolver::generateRandomBounds() {
    ...
    auto vec1Ptr = vec1->getLocalView<Kokkos::HostSpace>(Tpetra::Access::ReadWrite); // LeftLayout
    auto vec2Ptr = vec2->getLocalView<Kokkos::HostSpace>(Tpetra::Access::ReadWrite); // LeftLayout
