...
ConstraintOperator::ConstraintOperator(Teuchos::RCP<TOP> &mobOp_, Teuchos::RCP<TCMAT> &DMatTransRcp_,
    ...
    // explicit transpose
    {
        ...
        Tpetra::RowMatrixTransposer<double, LO, GO> transposerDu(DMatTransRcp);
...
void ConstraintOperator::apply(const TMV &X, TMV &Y, Teuchos::ETransp mode, scalar_type alpha, scalar_type beta) const {
    ...
    for (int i = 0; i < numVecs; i++) {
        ...
        auto XcolPtr = XcolRcp->getLocalView<Kokkos::HostSpace>(Tpetra::Access::ReadOnly);
        auto YcolPtr = YcolRcp->getLocalView<Kokkos::HostSpace>(Tpetra::Access::ReadWrite);
        auto invKappaPtr = invKappa->getLocalView<Kokkos::HostSpace>(Tpetra::Access::ReadOnly);
