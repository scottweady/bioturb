...
void CPSolver::clipZero(Teuchos::RCP<TV> &vecRcp) const {
    auto x_2d = vecRcp->getLocalView<Kokkos::HostSpace>(Tpetra::Access::ReadWrite); // LeftLayout
...
void CPSolver::maxXY(const Teuchos::RCP<const TV> &vecXRcp, const Teuchos::RCP<const TV> &vecYRcp,
    ...
    auto x_2d = vecXRcp->getLocalView<Kokkos::HostSpace>(Tpetra::Access::ReadOnly);
    auto y_2d = vecYRcp->getLocalView<Kokkos::HostSpace>(Tpetra::Access::ReadOnly);
    auto z_2d = vecZRcp->getLocalView<Kokkos::HostSpace>(Tpetra::Access::ReadWrite);
    for (size_t c = 0; c < x_2d.extent(1); c++) {
#pragma omp parallel for
        for (size_t i = 0; i < x_2d.extent(0); i++) {
...
// Y=Ax
    ...
    auto x_2d = vecXRcp->getLocalView<Kokkos::HostSpace>(Tpetra::Access::ReadOnly);
    auto y_2d = vecYRcp->getLocalView<Kokkos::HostSpace>(Tpetra::Access::ReadOnly);
    auto b_2d = vecbRcp->getLocalView<Kokkos::HostSpace>(Tpetra::Access::ReadOnly);
    auto z_2d = vecTempRcp->getLocalView<Kokkos::HostSpace>(Tpetra::Access::ReadWrite);
    for (size_t c = 0; c < x_2d.extent(1); c++) {
#pragma omp parallel for
        for (size_t i = 0; i < x_2d.extent(0); i++) {
...
void CPSolver::hMinMap(const Teuchos::RCP<const TV> &xRcp, const Teuchos::RCP<const TV> &yRcp,
    ...
    auto xView = xRcp->getLocalView<Kokkos::HostSpace>(Tpetra::Access::ReadOnly);
    auto yView = yRcp->getLocalView<Kokkos::HostSpace>(Tpetra::Access::ReadOnly);
    auto hView = hRcp->getLocalView<Kokkos::HostSpace>(Tpetra::Access::ReadWrite);
    auto maskView = maskRcp->getLocalView<Kokkos::HostSpace>(Tpetra::Access::ReadWrite);
...
CPSolver::CPSolver(int localSize, double diagonal) {
    ...
    if (commRcp->getRank() == 0) {
        ...
        std::cout << "local size: " << mapRcp->getLocalNumElements() << std::endl;
...
class mmNewtonOperator : public TOP {
  ...
  public:
    ...
    void apply(const TMV &X, TMV &Y, Teuchos::ETransp mode = Teuchos::NO_TRANS,
        ...
        const int localSize = X.getMap()->getLocalNumElements();
        // step 2  apply the mask

        auto xView = X.getLocalView<Kokkos::HostSpace>(Tpetra::Access::ReadOnly);
        auto yView = Y.getLocalView<Kokkos::HostSpace>(Tpetra::Access::ReadWrite);
        auto maskView = maskRcp->getLocalView<Kokkos::HostSpace>(Tpetra::Access::ReadOnly);
        for (size_t c = 0; c < xView.extent(1); c++) {
            for (size_t i = 0; i < xView.extent(0); i++) {
                if (maskView(i, c) > 0.5) {
                    yView(i, c) = xView(i, c);
...
int CPSolver::test_LCP(double tol, int maxIte, int solverChoice) {
    ...
    // dumpTV(xsolRcp, "xsol");
    // dumpTV(AxbRcp, "Axb");
    auto xView = xsolRcp->getLocalView<Kokkos::HostSpace>(Tpetra::Access::ReadOnly);
    auto yView = AxbRcp->getLocalView<Kokkos::HostSpace>(Tpetra::Access::ReadOnly);

    int xerrorN = 0;
    double xerrormax = 0.0;
    int yerrorN = 0;
    double yerrormax = 0.0;
    assert(xView.extent(0) == mapRcp->getLocalNumElements());
    for (size_t c = 0; c < xView.extent(1); c++) {
        for (size_t i = 0; i < xView.extent(0); i++) {
            if (xView(i, c) < 0) {
                xerrorN++;
