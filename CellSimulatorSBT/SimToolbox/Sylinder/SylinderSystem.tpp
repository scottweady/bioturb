    ...
    if (commRcp->getRank() != 0) {
        ...
        for (int i = 0; i < runConfig.sylinderNumber; i++) {
            ...
            for (int iQuadPt = 0; iQuadPt < N; iQuadPt++) {
                ...
                uinfHydro.col(iQuadPt) = Evec3(0, 0, 0);
                forceHydro.col(iQuadPt) = Evec3(0, 0, 0);
            }
            EmatmapN(newBody.uinfHydro).block(0, 0, 3, N) = uinfHydro;
            EmatmapN(newBody.forceHydro).block(0, 0, 3, N) = forceHydro;
...
void SylinderSystem<N>::calcMobMatrix() {
    ...
    const int nLocal = sylinderMapRcp->getLocalNumElements();
...
#pragma omp parallel for
    for (int i = 0; i < nLocal; i++) {
        ...
        Emat3 qq = q * q.transpose();
        Emat3 Imqq = Emat3::Identity() - qq;
        Emat3 Ipqq = Emat3::Identity() + qq;

        const double length = sy.length;
        const double diameter = sy.radius * 2;       
        const double slenderBodyCoeff = log(2 * length / diameter) / (4.0 * Pi * mu);
        MobTrans = slenderBodyCoeff * Ipqq;
        MobRot = Emat3::Identity() * 12 * slenderBodyCoeff / (length * length * length);
...
void SylinderSystem<N>::calcVelocityNonCon() {
    ...
    auto velNCPtr = velocityNonConRcp->getLocalView<Kokkos::HostSpace>(Tpetra::Access::ReadWrite);
...
#pragma omp parallel for
        ...
        auto forcePtr = forcePartNonBrownRcp->getLocalView<Kokkos::HostSpace>(Tpetra::Access::ReadWrite);
#pragma omp parallel for
        for (int i = 0; i < nLocal; i++) {
            auto &sy = sylinderContainer[i];
            // torque
    ...
    if (!velocityPartNonBrownRcp.is_null()) {
        if (runConfig.monolayer) {
            auto velNBPtr = velocityPartNonBrownRcp->getLocalView<Kokkos::HostSpace>(Tpetra::Access::ReadWrite);
#pragma omp parallel for
            for (int i = 0; i < nLocal; i++) {
...
#pragma omp parallel for
    ...
    if (!velocityBrownRcp.is_null()) {
        if (runConfig.monolayer) {
            auto velBPtr = velocityBrownRcp->getLocalView<Kokkos::HostSpace>(Tpetra::Access::ReadWrite);
#pragma omp parallel for
            for (int i = 0; i < nLocal; i++) {
...
void SylinderSystem<N>::setForceNonBrown(const std::vector<double> &forceNonBrown) {
    ...
    TEUCHOS_ASSERT(sylinderMobilityMapRcp->getLocalNumElements() == 6 * nLocal);
...
void SylinderSystem<N>::saveForceVelocityConstraints() {
    ...
    auto velUniPtr = velocityUniRcp->getLocalView<Kokkos::HostSpace>(Tpetra::Access::ReadOnly);
    auto velBiPtr = velocityBiRcp->getLocalView<Kokkos::HostSpace>(Tpetra::Access::ReadOnly);
    auto forceUniPtr = forceUniRcp->getLocalView<Kokkos::HostSpace>(Tpetra::Access::ReadOnly);
    auto forceBiPtr = forceBiRcp->getLocalView<Kokkos::HostSpace>(Tpetra::Access::ReadOnly);
...
#pragma omp for
    ...
    auto velocityPtr = velocityBrownRcp->getLocalView<Kokkos::HostSpace>(Tpetra::Access::ReadWrite);
