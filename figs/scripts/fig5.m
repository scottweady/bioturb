...
for i = 2 : 4
  ...
  % legend('location', 'northeast', 'color', 'none', 'FontSize', 18)
  sp{i - 1, 2}.TickLabelInterpreter = 'latex';
  sp{i - 1, 2}.FontSize = 18;
  sp{i - 1, 2}.Ticks = [25 150];

  ax = gca;

  ylim([-0.1 * max(ax.YLim) max(ax.YLim)])

%   ax = gca
  
%   ylabel(hb, '$L$', 'interpreter', 'latex', 'rotation', 0)
...
polar = @(L) recolorImage(imread(sprintf('../snapshots/polar/nu1.0_L%d.png', L)));
nematic = @(L) recolorImage(imread(sprintf('../snapshots/nematic/nu1.0_L%d.png', L)));
...
function render(nu, L, col, i)
  ...
  subplot(3, 4, 7 + i)
  plot(r(2 : end), Cbar(2 : end), 'o-', 'Color', col, 'DisplayName', sprintf('$L = %d$', L))
  xlim([0 20])
  hold on
